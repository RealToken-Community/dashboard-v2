...
export const AssetPageIssuesTab: FC<{
  ...
  const { t: tAssetIssues } = useTranslation('common', { keyPrefix: 'assetIssues' })
    ...
    <>
      <AssetPageTable
        data={[
          ...
          {
            label: t('priority'),
            value: realtoken.extraData?.pitsBI?.actions?.priority === undefined ?
          ...
          },
          {
            label: t('lawsuit'),
            value:
              realtoken.extraData?.pitsBI?.actions?.exhibit_number && (realtoken.extraData?.pitsBI?.actions?.volume) ?
