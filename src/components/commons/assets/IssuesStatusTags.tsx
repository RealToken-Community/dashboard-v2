...
const StatusIcons = forwardRef<
  HTMLDivElement,
  { value: string | undefined; priority: number | undefined }
>((props, ref) => {
      ...
      : value === RealTokenToBeFixedStatus.NoExhibit
        ...
        : value === RealTokenToBeFixedStatus.Scheduled
          ...
          : value === RealTokenToBeFixedStatus.UpgradedAndReady
            ...
            : 'purple'
  const icon =
    value === RealTokenToBeFixedStatus.NoExhibit || value == 'na' ? (
      <IconCheck size={16} color={iconColor} />
    ) : value === RealTokenToBeFixedStatus.Scheduled ? (
      <IconCalendarTime size={16} color={iconColor} />
    ) : value === RealTokenToBeFixedStatus.UpgradedAndReady ? (
      <IconHomeCheck size={16} color={iconColor} />
    ) : (
 ...
 */
export const IssueStatusTag: FC<{
 ...
 */
// eslint-disable-next-line react/display-name
const PriorityIcons = forwardRef<HTMLDivElement, { value: number | undefined }>(
  (props, ref) => {
    ...
    const prioritySize = !value ? 16 : 22
    const iconSize = 16
    const iconPriority = !value ? (
      value === 0 ? (
        <IconCheck size={prioritySize} color={piorityColor} />
      ) : (
        <></>
      )
    ) : value === 1 ? (
      <IconCircleNumber1 size={prioritySize} color={piorityColor} />
    ) : value === 2 ? (
      <IconCircleNumber2 size={prioritySize} color={piorityColor} />
    ) : (
      <IconCircleNumber3 size={prioritySize} color={piorityColor} />
    )
    const icon =
      value === 0 ? (
        <></>
      ) : value === 1 ? (
        <IconTrafficCone size={iconSize} color={iconColor} />
      ) : value === 2 ? (
        <IconTool size={iconSize} color={iconColor} />
      ) : (
        <IconHammer size={iconSize} color={iconColor} />
      )
 ...
 */
export const PriorityStatusTag: FC<{ value: number | undefined }> = ({
  value,
}) => {
  const { t } = useTranslation('common', { keyPrefix: 'assetIssues' })
  const isLoadingExtraData = useSelector(selectRealtokensIsLoadingExtraData)
...
>((props, ref) => {
  ...
  const noExhibit =
    (!exhibitNumber && !exhibitVolume) ||
    (exhibitNumber === -1 && exhibitVolume === -1)
  const iconColor = noExhibit
    ? 'gray'
    : priority === 1
      ? 'red'
      : priority === 2
  ...
  const icon =
    !exhibitNumber && !exhibitVolume ? (
      exhibitNumber === 0 && exhibitVolume === 0 ? (
        <IconFileOff size={iconSize} color={iconColor} />
      ) : (
        <IconFileUnknown size={iconSize} color={iconColor} />
      )
    ) : (
      <IconFileAlert size={iconSize} color={iconColor} />
    )
  return (
    <div ref={ref} {...rest}>
      <div style={{ display: 'flex', gap: '5px', alignItems: 'center' }}>
...
export const ExhibitStatusTag: FC<{
  ...
  exhibitVolume: number | undefined
  priority: number | undefined
}> = ({ exhibitNumber, exhibitVolume, priority }) => {
  const { t } = useTranslation('common', { keyPrefix: 'assetIssues' })
  const isLoadingExtraData = useSelector(selectRealtokensIsLoadingExtraData)
  const toolTipText =
    exhibitNumber === 0 && exhibitVolume === 0
      ...
      : !exhibitNumber && !exhibitVolume
        ...
        : exhibitNumber === -1 && exhibitVolume === -1
          ? t('lawsuit.na')
          : t('lawsuit.exhibit') + ` # ${exhibitNumber} volume ${exhibitVolume}`
  return (
    <>
      {isLoadingExtraData ? (
        <Skeleton width={30} height={20} />
